
CompactRaidDB = {
	["keepgroupstogether"] = 1,
	["showtooltip"] = 1,
	["nameColor"] = "1.0,1.0,1.0",
	["clickDownMode"] = 1,
	["showParty"] = 1,
	["modules"] = {
		["Artwork"] = {
			["font"] = "Fonts\\ARKai_T.ttf",
			["background"] = "Interface\\DialogFrame\\UI-DialogBox-Background",
			["border"] = "Interface\\Tooltips\\UI-Tooltip-Border",
			["statusbar"] = "Interface\\BUTTONS\\WHITE8X8.BLP",
		},
		["RaidDebuff"] = {
			["scale"] = 100,
			["userLevels"] = {
			},
			["selTier"] = 8,
			["customDebuffs"] = {
				["8,1178,302274"] = "2355,3",
				["8,1178,209858"] = "0,2",
				["8,1178,294929"] = "2339,5",
				["8,1178,240443"] = "0,2",
				["8,1036,290085"] = "0,2",
			},
			["xoffset"] = 0,
			["selBoss"] = 2339,
			["selInstance"] = 1178,
			["yoffset"] = 0,
		},
	},
	["powerColor"] = "0.0,0.0,1.0",
	["groupSwapNoNotify"] = 1,
	["showToolboxes"] = 1,
	["outrangeAlpha"] = 40,
	["showRoleIcon"] = 1,
	["showDebuffs"] = 1,
	["nameWidthLimit"] = 75,
	["showDispels"] = 1,
	["version"] = 5.09,
	["showbarbkgnd"] = 1,
	["height"] = 40,
	["showPrivIcons"] = 1,
	["nameYOffset"] = 0,
	["tooltipPosition"] = 0,
	["scale"] = 100,
	["showBuffs"] = 1,
	["spacing"] = 1,
	["showRaidIcon"] = 1,
	["containerBorderSize"] = 12,
	["profiles"] = {
		["维维尔 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent1"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["我找不到了 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
					["talent3"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
					["talent1"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["Ouse - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["白白净净啥 - 古尔丹"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
					},
				},
				["ClickSets"] = {
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["ctrl-2"] = "buildin:氤氲之雾",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["ctrl-2"] = "buildin:氤氲之雾",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["落月复西斜 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["2"] = "action:togglemenu",
						["ctrl-2"] = "buildin:氤氲之雾",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["2"] = "action:togglemenu",
						["ctrl-2"] = "buildin:氤氲之雾",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["2"] = "action:togglemenu",
						["ctrl-2"] = "buildin:氤氲之雾",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
		},
		["我会消失 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:暗影步",
						["ctrl-1"] = "buildin:嫁祸诀窍",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["alt-1"] = "buildin:暗影步",
						["ctrl-1"] = "buildin:嫁祸诀窍",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:暗影步",
						["ctrl-1"] = "buildin:嫁祸诀窍",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
					},
					["talent3"] = {
					},
					["talent1"] = {
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["我的小胖狗 - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent1"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#治疗宠物#[style]#2#",
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#误导#",
					},
				},
				["ClickSets"] = {
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:误导",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["弄妆梳洗迟 - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["我会飞你会吗 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent4"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["2"] = "action:togglemenu",
					},
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["shift-1"] = "buildin:愈合",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["2"] = "action:togglemenu",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
				},
				["CornerIndicators"] = {
					["talent4"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["RIGHT"] = "[selfcast]#1#[aura]#栽培#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[style]#2#[r1]#0.0#[r2]#1.0#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOP"] = "[selfcast]#1#[scale]#150#[aura]#艾欧娜尔之种#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[style]#2#[r1]#0.0#[r2]#1.0#",
						["LEFT"] = "[selfcast]#1#[aura]#回春术（萌芽）#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[style]#2#[r1]#0.0#[r2]#1.0#",
					},
					["talent2"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
				},
			},
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["Mubbh - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent3"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#真言术：盾#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#恢复#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#真言术：盾#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#恢复#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#",
					},
				},
				["ClickSets"] = {
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-2"] = "buildin:真言术：盾",
						["alt-ctrl-1"] = "buildin:守护之魂",
						["2"] = "action:togglemenu",
						["ctrl-1"] = "buildin:恢复",
						["alt-2"] = "buildin:愈合祷言",
						["shift-2"] = "buildin:纯净术",
						["shift-1"] = "buildin:快速治疗",
						["alt-1"] = "buildin:治疗术",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-2"] = "buildin:真言术：盾",
						["alt-ctrl-1"] = "buildin:守护之魂",
						["2"] = "action:togglemenu",
						["ctrl-1"] = "buildin:恢复",
						["alt-2"] = "buildin:愈合祷言",
						["shift-2"] = "buildin:纯净术",
						["alt-1"] = "buildin:治疗术",
						["shift-1"] = "buildin:快速治疗",
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["朝飞暮卷 - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
				},
				["ClickSets"] = {
					["talent3"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["斜月藏海雾 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["shift-1"] = "buildin:愈合",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["shift-1"] = "buildin:愈合",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["2"] = "action:togglemenu",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
		},
		["弄妆梳洗迟 - 古尔丹"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["风起春城暮 - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent4"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
					["talent2"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
				},
				["ClickSets"] = {
					["talent_Synced"] = {
						["alt-1"] = "buildin:复生",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
					["talent2"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
					["talent3"] = {
						["alt-1"] = "buildin:复生",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
					["talent1"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
				},
			},
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["Funerals - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
					},
					["talent3"] = {
					},
					["talent1"] = {
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复活盟友",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复活盟友",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复活盟友",
						["2"] = "action:togglemenu",
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["高楼鼓角悲 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent1"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["Cleaner - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["2"] = "action:togglemenu",
						["ctrl-2"] = "buildin:氤氲之雾",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["2"] = "action:togglemenu",
						["ctrl-2"] = "buildin:氤氲之雾",
					},
				},
				["CornerIndicators"] = {
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["呜呜哇哇 - 古尔丹"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
				},
				["ClickSets"] = {
					["talent3"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 4.33,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["她的眼眶里 - 古尔丹"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent1"] = {
					},
					["talent2"] = {
					},
				},
				["ClickSets"] = {
					["talent1"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent2"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["我会变成狼 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:治疗链",
						["ctrl-2"] = "buildin:激流",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:净化灵魂",
						["1"] = "action:target",
						["ctrl-1"] = "buildin:大地之盾",
					},
					["talent3"] = {
						["alt-1"] = "buildin:治疗链",
						["ctrl-2"] = "buildin:激流",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:净化灵魂",
						["1"] = "action:target",
						["ctrl-1"] = "buildin:大地之盾",
					},
					["talent1"] = {
						["alt-1"] = "buildin:治疗链",
						["ctrl-2"] = "buildin:激流",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:净化灵魂",
						["1"] = "action:target",
						["ctrl-1"] = "buildin:大地之盾",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#激流#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#大地之盾#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#激流#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#大地之盾#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#激流#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#大地之盾#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["Weeping - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
					["talent3"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
					["talent1"] = {
						["alt-1"] = "buildin:圣光道标",
						["ctrl-2"] = "buildin:神圣震击",
						["alt-ctrl-1"] = "buildin:救赎",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:清洁术",
						["1"] = "action:target",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[selfcast]#1#[aura]#圣光道标#[style]#2#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["弄妆梳洗迟 - 死亡之翼"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
					},
					["talent1"] = {
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["我会招宝宝 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:误导",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:误导",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:误导",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#误导#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#治疗宠物#[style]#2#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#误导#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#治疗宠物#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#误导#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#治疗宠物#[style]#2#",
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["坎坎坷坷 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent1"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["我喝了假酒 - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#氤氲之雾#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#复苏之雾#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#作茧缚命#",
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["ctrl-2"] = "buildin:氤氲之雾",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["ctrl-2"] = "buildin:氤氲之雾",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:复苏之雾",
						["alt-ctrl-1"] = "buildin:作茧缚命",
						["shift-1"] = "buildin:抚慰之雾",
						["ctrl-2"] = "buildin:氤氲之雾",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["十里扬州路 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
					},
					["talent1"] = {
					},
				},
			},
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["我会招恶魔 - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
					},
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#灵魂石#",
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#无尽呼吸#",
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:无尽呼吸",
						["ctrl-1"] = "buildin:灵魂石",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["Dremel - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:误导",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#误导#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#治疗宠物#[style]#2#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["十里扬州路 - 古尔丹"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent1"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent2"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent1"] = {
					},
					["talent2"] = {
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["回首见旌旗 - 亡语者"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent4"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
				},
				["ClickSets"] = {
					["talent4"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["shift-1"] = "buildin:愈合",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["shift-1"] = "buildin:愈合",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["我会大跳你呢 - 亡语者"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent3"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent2"] = {
					},
					["talent3"] = {
					},
					["talent1"] = {
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["Believer - 古尔丹"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent4"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["shift-1"] = "buildin:愈合",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["2"] = "action:togglemenu",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["2"] = "action:togglemenu",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
				},
				["CornerIndicators"] = {
					["talent4"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
					["talent2"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
				},
			},
			["showPartyPets"] = 1,
			["version"] = 5.09,
			["showFriendlyNpc"] = 1,
		},
		["不不尾王 - 古尔丹"] = {
			["modules"] = {
				["ClickSets"] = {
					["talent3"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
				["CornerIndicators"] = {
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["懒起画蛾眉 - 古尔丹"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent2"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#真言术：盾#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#恢复#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[style]#2#[r1]#0.0#[r2]#1.0#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#真言术：盾#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#恢复#[style]#2#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[ignoreVehicle]#1#[aura]#真言术：盾#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#救赎#[b1]#0.0#[b2]#0.0#[b3]#0.0#[g1]#1.0#[g3]#0.0#[g2]#1.0#[r3]#1.0#[style]#2#[r1]#0.0#[r2]#1.0#",
						["BOTTOMLEFT"] = "[ignoreVehicle]#1#[aura]#愈合祷言#",
					},
				},
				["ClickSets"] = {
					["talent2"] = {
						["1"] = "action:target",
						["ctrl-2"] = "buildin:真言术：盾",
						["alt-ctrl-1"] = "buildin:守护之魂",
						["2"] = "action:togglemenu",
						["ctrl-1"] = "buildin:恢复",
						["alt-2"] = "buildin:愈合祷言",
						["shift-2"] = "buildin:纯净术",
						["alt-1"] = "buildin:治疗术",
						["shift-1"] = "buildin:快速治疗",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-2"] = "buildin:真言术：盾",
						["alt-ctrl-1"] = "buildin:守护之魂",
						["2"] = "action:togglemenu",
						["ctrl-1"] = "buildin:恢复",
						["alt-2"] = "buildin:愈合祷言",
						["shift-2"] = "buildin:纯净术",
						["alt-1"] = "buildin:治疗术",
						["shift-1"] = "buildin:快速治疗",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-2"] = "buildin:真言术：盾",
						["alt-ctrl-1"] = "buildin:守护之魂",
						["2"] = "action:togglemenu",
						["ctrl-1"] = "buildin:恢复",
						["alt-2"] = "buildin:愈合祷言",
						["shift-2"] = "buildin:纯净术",
						["alt-1"] = "buildin:治疗术",
						["shift-1"] = "buildin:快速治疗",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["十里扬州路 - 安苏"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent3"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
					["talent1"] = {
						["TOPLEFT"] = "[ignoreVehicle]#1#[aura]#缓落术#",
					},
				},
				["ClickSets"] = {
					["talent3"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["alt-1"] = "buildin:缓落术",
						["1"] = "action:target",
						["2"] = "action:togglemenu",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
		["Believe - 古尔丹"] = {
			["modules"] = {
				["CornerIndicators"] = {
					["talent4"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
					},
					["talent3"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
					["talent1"] = {
						["TOPRIGHT"] = "[selfcast]#1#[aura]#野性成长#[style]#2#",
						["TOPLEFT"] = "[selfcast]#1#[aura]#回春术#[style]#2#",
						["BOTTOMLEFT"] = "[selfcast]#1#[aura]#生命绽放#[style]#2#",
					},
				},
				["ClickSets"] = {
					["talent4"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["2"] = "action:togglemenu",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["ctrl-2"] = "buildin:生命绽放",
					},
					["talent3"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["shift-1"] = "buildin:愈合",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["2"] = "action:togglemenu",
					},
					["talent1"] = {
						["1"] = "action:target",
						["ctrl-1"] = "buildin:回春术",
						["ctrl-2"] = "buildin:生命绽放",
						["2"] = "action:togglemenu",
						["shift-2"] = "buildin:自然之愈",
						["alt-ctrl-2"] = "buildin:野性成长",
						["alt-2"] = "buildin:迅捷治愈",
						["shift-1"] = "buildin:愈合",
					},
				},
			},
			["version"] = 5.09,
			["showPartyPets"] = 1,
			["showFriendlyNpc"] = 1,
		},
	},
	["onlyDispellable"] = 1,
	["healthColor"] = "0.0,1.0,0.0",
	["raidFilter"] = "CLASS",
	["unitBkColor"] = "0.0,0.0,0.0",
	["width"] = 76,
	["nameHeight"] = 12,
	["showDirectionArrow"] = 1,
	["powerBarHeight"] = 5,
	["showSolo"] = 1,
	["nameXOffset"] = 0,
	["healthtextmode"] = 0,
	["containerAlpha"] = 75,
	["invertColor"] = 1,
}
